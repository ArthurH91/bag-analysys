...
from read_plot_utilsv2 import read_jsid_bag
...
params = croco_reach.get_croco_reaching()
T = params["nb_shooting_nodes"]
...
def get_pose_list(q_array):
    ...
    for i, q in enumerate(q_array):
        pose_list[i, :] = model.placement(q.copy(), model.nq).translation.copy()
...
def plotting():
    for i_field, field in enumerate(fields):
        fig_dq, ax_dq = plt.subplots(model.nq, 1)
        fig_tau, ax_tau = plt.subplots(model.nq, 1)
